...
--                    WHERE MEM_NAME LIKE '김%';
        ...
        예) 주소지가 서울인 회원들의 회원번호, 회원명, 주소를 검색하시오
        ...
        예) 주소지가 대전이고, 직업이 자영업이며, 남자인 회원들의 회원번호, 회원명, 주소를 검색하시오.
             SELECT MEM_ID      AS  회원번호,
                    MEM_NAME    AS  회원명,
                    MEM_ADD1 || ' ' || MEM_ADD2 AS 주소
                    FROM MEMBER 
                WHERE MEM_ADD1 LIKE '대전%' AND MEM_JOB = '자영업' AND MEM_REGNO2 LIKE '1%';
            
             
        (4) EXISTS 연산자
            - IN과 비슷한 기능을 수행    -- IN은 ,랑 서브쿼리도 가능
            - 조건절에 값리스트를 사용하지 않고 서브쿼리만 사용   -- ,로 비교한 값 말고 서브쿼리만 가능.
            (사용형식)
            EXISTS(서브쿼리)
        ...
        예) 
         SELECT  A.EMPLOYEE_ID AS 사원번호,
                ...
                A.SALARY  AS 급여
            FROM EMPLOYEES A
        WHERE EXISTS (SELECT 1 FROM EX_01 B
          ...
          SELECT  EMPLOYEE_ID AS 사원번호,
            ...
            SALARY  AS 급여
        FROM EMPLOYEES
        WHERE EMPLOYEE_ID IN(SELECT EMPLOYEE_ID FROM EX_01);
        
        (조인을 이용한 방법)
        ALTER TABLE EX_01
        ADD CONSTRAINT EX_PK PRIMARY KEY(EMPLOYEE_ID);
        
        ALTER TABLE EX_01
        ADD CONSTRAINT EX_FK FOREIGN KEY(EMPLOYEE_ID)
       ...
       SELECT  EMPLOYEES.EMPLOYEE_ID AS 사원번호,
            ...
            SALARY  AS 급여
        FROM EMPLOYEES
        JOIN EX_01 ON EMPLOYEES.EMPLOYEE_ID = EX_01.EMPLOYEE_ID;
